...
function en_side_update()
	...
	if(count(en_side) < 2) then
		if(flr(rnd(20)) == 10) then
			add(en_side, {x = camx + 1, y = en_y + 2, type = 1, spd = levels[currentlevel].en_side_spd_max, anim = 0})
		end
		if(flr(rnd(20)) == 5) then 
			add(en_side, {x = camx + 1, y = en_y - 4, type = 2, spd = levels[currentlevel].en_side_spd_min, anim = 0})
		end
	end
